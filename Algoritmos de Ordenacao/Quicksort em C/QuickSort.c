...
struct Lista{
    ...
    char arrayCabelo[50][50];
    char arrayNome[50][50];
    int n;
};
...
void construirArrayCabelo(struct Lista* lista){
    for(int i = 0; i < lista->n; i++){
        Personagem personagem = lista->array[i];
        strcpy(lista->arrayCabelo[i], personagem.corDoCabelo);
    }
}

void construirArrayNome(struct Lista* lista){
    for(int i = 0; i < lista->n; i++){
        Personagem personagem = lista->array[i];
        strcpy(lista->arrayNome[i], personagem.nome);
    }
}
...
void swap(int a, int b, struct Lista* lista){//trocar posicao a com posicao b
    ...
    strcpy(nomeTmp, lista->arrayNome[a]);
    strcpy(lista->arrayNome[a], lista->arrayNome[b]);
    strcpy(lista->arrayNome[b], nomeTmp);

    strcpy(cabeloTmp, lista->arrayCabelo[a]);
    strcpy(lista->arrayCabelo[a], lista->arrayCabelo[b]);
    strcpy(lista->arrayCabelo[b], cabeloTmp);
}

void QuickSortNome(int esq, int dir, struct Lista* lista){
    int i = esq;
    int j = dir;
    char pivo[100];
    int aux = 0;
    strcpy(pivo, lista->arrayNome[(dir+esq)/2]);

    while(i <= j){
        while (strcmp(lista->arrayNome[i], pivo) < 0){
            i++;
        }
        while (strcmp(lista->arrayNome[j], pivo) > 0){
            j--;
        }
        if (i <= j){
            swap(i, j, lista);
    ...
    if (esq < j){
        QuickSortNome(esq, j, lista);
    }
    if (i < dir){
        QuickSortNome(i, dir, lista);
    }
}

void QuickSortCabelo(int esq, int dir, struct Lista* lista){
    int i = esq;
    int j = dir;
    char pivo[100];
    int aux = 0;
    strcpy(pivo, lista->arrayCabelo[(dir+esq)/2]);

    while(i <= j){
        while (strcmp(lista->arrayCabelo[i], pivo) < 0){
            i++;
        }
        while (strcmp(lista->arrayCabelo[j], pivo) > 0){
            j--;
        }
        if (i <= j){
            swap(i, j, lista);
    ...
    if (esq < j){
        QuickSortCabelo(esq, j, lista);
    }
    if (i < dir){
        QuickSortCabelo(i, dir, lista);
    }
}

void verificarCabelosIguais(struct Lista* lista){
    int inicio = 0;
    int fim = 0;
    int i = 0;

    while (i < lista->n){
        if (strcmp(lista->arrayCabelo[i], lista->arrayCabelo[i+1]) == 0){
            inicio = i;
            while (strcmp(lista->arrayCabelo[i], lista->arrayCabelo[i+1]) == 0){
                i++;
            }
            fim = i;
            QuickSortNome(inicio, fim, lista);
        }
        else{
            i++;
        }
...
int main (){
    ...
    construirArrayCabelo(&lista);
    QuickSortCabelo(0, lista.n-1, &lista);
    construirArrayNome(&lista);
    construirArrayCabelo(&lista);
    verificarCabelosIguais(&lista);
    mostrar(&lista);
    return 0;
}