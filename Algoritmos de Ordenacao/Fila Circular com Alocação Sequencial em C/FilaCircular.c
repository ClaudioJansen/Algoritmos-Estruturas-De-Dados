...
void inserirInicio(struct Personagem personagem, struct Fila* fila);
...
void inserir(Personagem personagem, struct Fila *fila){
    fila->array[fila->fim] = personagem;
    fila->fim = (fila->fim + 1) % 6;

    if(fila->fim == fila->inicio && fila->cont != 0.0){
        fila->inicio = (fila->inicio + 1) % 6;
        fila->cont--;
    }
    fila->cont++;
...
void imprimirRemocoes(int pos, struct Fila* fila){
    Personagem personagem = fila->array[pos];
    printf("(R) %s\n", personagem.nome);
}
...
int calcularMedia(struct Fila* fila){
    int media = 0;
    double frac = 0.0;
    fila->media = 0;

    for (int i = fila->inicio; i != fila->fim; i = (i+1) % 6){
        Personagem personagem = fila->array[i];
        fila->media += personagem.altura / fila->cont;
        
    }
    media = arredondar(fila->media);
...
int main(int argc, char *argv[]){
    ...
    for(int i = 0; i < n; i++){
        ...
        if(str_contains(' ', movimentacoes)){
            if (movimentacoes[0] == 'I'){
                int j = 0;
                for(int i = 2; i < strlen(movimentacoes); i++){
            ...
            imprimirRemocoes(fila.inicio, &fila);
            remover(&fila);
        }
    }
    return 0;
