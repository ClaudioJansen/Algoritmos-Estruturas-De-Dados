...
void swap(int a, int b, struct Lista *lista);
...
void imprimirPersonagem(struct Personagem personagem){  
    ...
    if (aux == 0.0){
        ...
        aux = aux*10;
        personagem.peso = personagem.peso + aux;
...
void construirArrayNome(struct Lista *lista, int i){
    if (i < lista->n){
        Personagem personagem = lista->array[i];
        strcpy(lista->arrayNome[i], personagem.nome);
        construirArrayNome(lista, i+1);
    }
}

void ordenarSelecao(struct Lista *lista, int i){
    if (i < (lista->n - 1)){
        char nome1[100];
        strcpy(nome1, lista->arrayNome[i]);
        int menor = i;
        for (int j = (i+1); j < lista->n; j++){
            char nome2[100];
            strcpy(nome2, lista->arrayNome[j]);
            if(strcmp(nome1, nome2) > 0){
                menor = j;
            }
        }
        swap(menor, i, lista);
        ordenarSelecao(lista, i+1);
    }
...
int main (){
    ...
    construirArrayNome(&lista, 0);
    ordenarSelecao(&lista, 0);
    mostrar(&lista);
    return 0;
}