...
void radixsort(struct Lista *lista) {
	int i, exp = 1, m = 0, bucket[lista->n], temp[lista->n];
    Personagem tmp[lista->n];

	for(i = 0; i < lista->n; i++) {
		if(lista->arrayAltura[i] > m) {
			m = lista->arrayAltura[i];
		}
	}
	...
	while((m/exp) > 0) {
		...
		for(i = 0; i < lista->n; i++) {
			bucket[(lista->arrayAltura[i] / exp) % 10]++;
		}
		for(i = 1; i < lista->n; i++) {
			bucket[i] += bucket[i-1];
		}
		for(i = (lista->n - 1); i >= 0; i--) {
			temp[--bucket[(lista->arrayAltura[i] / exp) % 10]] = lista->arrayAltura[i];
		}
		for(i = 0; i < lista->n; i++) {
			lista->arrayAltura[i] = temp[i];
		}
		exp *= 10;
	}
}

void ordenaPersonagem(struct Lista *lista){
    for (int i = 0; i < lista->n; i++){
        for (int j = i; j < lista->n; j++){
            Personagem personagem = lista->array[j];
            if (lista->arrayAltura[i] == personagem.altura){
                swap(i, j, lista);
                j = lista->n;
            }
        }
...
void verificarAlturasIguais(struct Lista* lista){
    int inicio = 0;
    int fim = 0;
    int i = 0;
...
int main (){
    ...
    //constroe array só com as alturas
    construirArrayAltura(&lista);
    //ordena o array de altura atravez do radixsort
    radixsort(&lista);
    //procura as respectivas alturas dos personagens comparando com o array de altura pra ordenar agora o array de personagem
    ordenaPersonagem(&lista);
    //constroe o array só com os nomes
    construirArrayNome(&lista);
    //verifica os personagens com alturas iguais para ordenar usando o atributo nome
    verificarAlturasIguais(&lista);
    mostrar(&lista);
    return 0;
}