...
void inserirInicio(struct Personagem personagem, struct Lista* lista);
...
void inserirInicio(Personagem personagem, struct Lista *lista){
    if(lista->n < MAX){
...
void inserir(Personagem personagem, int pos, struct Lista *lista){
...
void remover(int pos, struct Lista *lista){
...
void imprimirRemocoes(int pos, struct Lista *lista){
    Personagem personagem = lista->array[pos];
    printf("(R) %s\n", personagem.nome);
}
...
int main (){
    ...
    for(int i = 0; i < n; i++){
        ...
        if(str_contains(' ', movimentacoes)){
            char insRem[100];
            int cont = 0;
            int j = 0;
            char num[2];
            
            if (movimentacoes[0] == 'I' && movimentacoes[1] == 'I'){
                for(int i = 3; i < strlen(movimentacoes); i++){
                    arquivo[j] = movimentacoes[i];
                    j++;
                }
                arquivo[j] = '\0';
                personagem = lerPersonagem(arquivo);
                inserirInicio(personagem, &lista);
            }
            else if (movimentacoes[0] == 'I' && movimentacoes[1] == 'F'){
                for(int i = 3; i < strlen(movimentacoes); i++){
                    arquivo[j] = movimentacoes[i];
                    j++;
                }
            ...
            if (movimentacoes[0] == 'I' && movimentacoes[1] == '*'){
                num[0] = movimentacoes[3];
                
                if (movimentacoes[1] != ' '){
                    num[1] = movimentacoes[4];
                }
                pos = atoi(num);


                for(int i = 6; i < strlen(movimentacoes); i++){
                    arquivo[j] = movimentacoes[i];
                    j++;
                }
                arquivo[j] = '\0';
                personagem = lerPersonagem(arquivo);
                inserir(personagem, pos, &lista);
            }
            else if (movimentacoes[0] == 'R' && movimentacoes[1] == '*'){
                num[0] = movimentacoes[3];
                ...
                imprimirRemocoes(pos, &lista);
                remover(pos, &lista);
            }
        }
        else{
            if(movimentacoes[0] == 'R' && movimentacoes[1] == 'I'){
                imprimirRemocoes(0, &lista);
                removerInicio(&lista);
            }
            else if(movimentacoes[0] == 'R' && movimentacoes[1] == 'F'){
                imprimirRemocoes(lista.n-1, &lista);
                removerFim(&lista);
            }
        }
    }
