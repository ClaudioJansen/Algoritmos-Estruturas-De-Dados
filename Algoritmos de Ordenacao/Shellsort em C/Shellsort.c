...
void construirArrayPeso(struct Lista* lista){
    for (int i = 0; i < lista->n; i++){
        Personagem personagem = lista->array[i];
        lista->arrayPeso[i] = personagem.peso;
    }
}
...
void insercaoPorCor(int cor, int h, struct Lista* lista){
    for (int i = (h + cor); i < lista->n; i += h){
        double tmp = lista->arrayPeso[i];
        Personagem Ptmp = lista->array[i];
        int j = i - h;
        while ((j >= 0) && (lista->arrayPeso[j]) > tmp){
            lista->arrayPeso[j+h] = lista->arrayPeso[j];
            lista->array[j+h] = lista->array[j];
            j -= h;
        }
        lista->arrayPeso[j + h] = tmp;
        lista->array[j + h] = Ptmp;
    }
}

void shellSort(struct Lista* lista) {
    int h = 1;
...
void verificarPesosIguais(struct Lista* lista){
    int inicio = 0;
    int fim = 0;
    int i = 0;

    while (i < lista->n){
        if (lista->arrayPeso[i] == lista->arrayPeso[i+1]){
            inicio = i;
            while (lista->arrayPeso[i] == lista->arrayPeso[i+1]){
                i++;
            }
            fim = i;
...
int main (){
    ...
    //ordenei o array usando shellsort, depois usei quicksort para ordenar as partes que os pesos sao iguais
    construirArrayPeso(&lista);
    shellSort(&lista);
    construirArrayNome(&lista);
    verificarPesosIguais(&lista);
    mostrar(&lista);
    return 0;
}