...
class ArvoreTrie{
    ...
    public ArvoreTrie merge(ArvoreTrie a2) throws Exception{
        ArvoreTrie a3 = new ArvoreTrie();
        merge(this.raiz, "", a3);
        a3 = merge(a2.raiz, "", a3);
        return a3;
    }

    private ArvoreTrie merge(No no, String s, ArvoreTrie a3) throws Exception{

        if(no.folha == true){
            s = s.trim();
            a3.inserir(s+no.elemento);
        } 
        else{
            for(int i = 0; i < no.prox.length; i++){
                if(no.prox[i] != null){
                    merge(no.prox[i], s + no.elemento, a3);
                }
            }
        }
        return a3;
    ...
    public static void main(String[] args) throws Exception{
        ...
        while (aux < 1){
            ...
            if (nomeArquivo.charAt(0) == 'F' && nomeArquivo.charAt(1) == 'I' && nomeArquivo.charAt(2) == 'M'){
                ...
                av1.inserir(personagem.getNome());
            }
        }
        aux = 0;
        while (aux < 1){
            ...
            if (nomeArquivo.charAt(0) == 'F' && nomeArquivo.charAt(1) == 'I' && nomeArquivo.charAt(2) == 'M'){
                ...
                Personagem personagem = new Personagem();
                personagem.lerPersonagem(nomeArquivo);
                av2.inserir(personagem.getNome());
            }
        }
        ArvoreTrie nova = av1.merge(av2);
        aux = 0;
        while (aux < 1){
            nome = MyIO.readLine();
