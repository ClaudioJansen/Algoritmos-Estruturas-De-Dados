...
void start(struct Hash* tabela){
    for(int i = 0; i < tabela->tamanho; i++){
        Lista lista;
        Personagem personagem;
        lista.primeiro = novaCelula(personagem);
        lista.ultimo = lista.primeiro;
        tabela->array[i] = lista;
    }
}
...
void inserirFim(Lista *lista, Personagem x) {
    lista->ultimo->prox = novaCelula(x);
    lista->ultimo = lista->ultimo->prox;
}

bool pesquisar(Hash *tabela, char *nome){
    bool resp = false;

    for(int j = 0; j < tabela->tamanho; j++){
        Celula* i;
        for(i = tabela->array[j].primeiro->prox; !resp && i != NULL; i = i->prox){
...
int main(){
    ...
    while (aux < 1){
        ...
        if (nomeArquivo[0] == 'F' && nomeArquivo[1] == 'I' && nomeArquivo[2] == 'M'){
            ...
            int pos = personagem.altura % tabela.tamanho;
            inserirFim(&tabela.array[pos], personagem);
        }
    }
    aux = 0;
    while (aux < 1){
